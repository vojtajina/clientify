...
var overrideVersions = function(dependencies, override) {
  _.each(override, function(newConstraint, name) {
...
var initializeNpm = function(versionsToOverride) {
          ...
          cache[name] = registryGet(name).then(function(data) {
            ...
            _.each(multiVersionPkg.versions, function(pkg, version) {
              penging.push(resolveGitUrls(pkg.dependencies).then(function(resolvedDeps) {
                _.merge(resolvedDeps, versionsToOverride[name]);
...
module.exports = function(rootPackage, githubReposToLoad, versionsToOverride) {
  return q.all([
    initializeNpm(versionsToOverride),
    resolveGitUrls(rootPackage.dependencies),
    resolveGitUrls(rootPackage.devDependencies),
    prefetchGithubPackages(githubReposToLoad)
  ]).then(function(container) {
    var registry = container[0];
    var resolvedDeps = _.merge({}, container[1], container[2]);
